...
const SortAlgo = {
  ...
  bubbleSort() {
    ...
    async function sort(self) {
      ...
      svg.selectAll("rect").style("fill", "#33FF39");
    ...
    async function sort(self) {
      ...
      for (i = 1; i < data.length; i++) {
        ...
        while (j>=0 && data[j]>key) {
          ...
          changeBarColor(data[j], "#ff0000");
          await timer(time);

          data[j+1]=data[j];
          changeBarColor(data[j], "#FFC0CB");
      ...
      // After complete sorting algorithm making all the bar green.
      svg.selectAll("rect").style("fill", "#90EE90");
